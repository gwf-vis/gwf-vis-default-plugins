...
import { LocationSelection } from "../utils/basic";
...
export default class GWFVisPluginPrairieWaterChart
  ...
  connectedCallback() {
    ...
    this.shouldShowChart = this.checkIfPluginIsInTheLargePresenterDelegate?.() ?? false;
  }
  ...
  render() {
    return html`
            ${when(this.shouldShowChart, () => html`
            <iframe src=${`http://51.79.85.49/advanced-chart.php`} width="100%" height="100%" frameborder="0"></iframe>
            `, () => html`
                <h3>Selected Location:</h3>
                <br/>
                <span>Data Source: <b>${this.locationSelection?.dataSource ?? 'N/A'}</b></span>
                <br/>
                <span>Location ID: <b>${this.locationSelection?.locationId ?? 'N/A'}</b></span>
                <div style="color: red; font-size: 2em;">Enter large view to see the chart.</div>
            `)}
        `;
  }
}